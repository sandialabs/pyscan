...
def load_experiment(file_name):
    ...
    if data_version == 0.1:
        ...
        generated = generated_datasets(expt.runinfo)
        measured = set(all_datasets).symmetric_difference(generated)
        expt.runinfo.measured = list(measured)
        f.close()

        return expt


def generated_datasets(runinfo):
    """
    HDF5 files contain two types of datasets.  Measured datasets are data from
    the measure_function, and were initially included in runinfo.measured.
    Pyscan generated datasets are created when the experiment runs. This
    function identifies the generated dataset from each scan.
