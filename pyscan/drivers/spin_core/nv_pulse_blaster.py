...
class NVPulseBlaster(PulseBlasterESRPro500):
    ...
    def setup_rabiJ(self, pi2, awg_lag, n_cycles,
                    duty_cycle_pi2=None,
                    off_time=500 * ns,
                    init_time=15 * us,
                    print_sequence=False):

        if duty_cycle_pi2 is None:
            duty_cycle_pi2 = pi2
        ...
        aom += 2**self.aom
        aom_and_daq = 0
        aom_and_daq += 2**self.aom
        aom_and_daq += 2**self.daq

        self.total_time = (pi2 + init_time + off_time + awg_lag)
        duty_cycle_time = (duty_cycle_pi2 + init_time + off_time + awg_lag)

        if duty_cycle_time > self.total_time:
            off_time = off_time + (duty_cycle_time - self.total_time)
            self.total_time = duty_cycle_time
        if pi2 == 0:
            mw_on = all_off
        ...
        self.start_programming()
        self.inst(all_off, Inst.LOOP, n_cycles, 100)
        self.inst(aom, Inst.CONTINUE, 0, init_time)
        self.inst(0, Inst.CONTINUE, 0, awg_lag)
        self.inst(mw_on, Inst.CONTINUE, 0, pi2 + 20)
        self.inst(all_off, Inst.CONTINUE, 0, 3000)
        self.inst(aom_and_daq, Inst.CONTINUE, 0, init_time)
        self.inst(all_off, Inst.CONTINUE, 0, off_time)

        self.inst(aom, Inst.CONTINUE, 0, init_time)
        self.inst(all_off, Inst.CONTINUE, 0, awg_lag)
        self.inst(all_off, Inst.CONTINUE, 0, pi2 + 20)
        self.inst(all_off, Inst.CONTINUE, 0, 3000)
        self.inst(aom_and_daq, Inst.CONTINUE, 0, init_time)
        self.inst(all_off, Inst.END_LOOP, 0, off_time)
        self.inst(all_off, Inst.STOP, 0, 100)
        self.stop_programming()

        if print_sequence:
            print(mw_on, pi2)
            print(aom_and_daq, init_time)
            print(all_off, off_time)
