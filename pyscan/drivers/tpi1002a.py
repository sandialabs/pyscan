...
from math import sin,cos,acos,atan2,sqrt,floor,log
import numpy as np
...
def print_error(errcode):
    print("TPI Error: %s"%['Checksum error','Undefined command type (first body byte)','Undefined command (second body byte)','Data out of range','Illegal beacon message character','Requested RF level < -90 dBm','Internal beacon message length error','Unknown script command','No detector available','No auxiliary input available','No trigger output available','Communication watchdog timeout','Failed to write EEPROM','Failed to read EEPROM'][errcode])
    return 
...
class TPI1002A(InstrumentDriver):
    ...
    '''
    # https://www.dropbox.com/sh/pzle1yanmlh3cnq/AAC2AZpp7RyDYgZzvBxwj2RJa?dl=0&preview=AN_2_TPI_User_Command_Structure_rev_1.8.pdf
    ...
    def read(self,return_bytes=None,raw=False):
        """Set nbytes (without header/checksum) to expected number of bytes.  If you're right, things will be fast.  If you're wrong, comm will be out of sync."""
        if return_bytes!=None:
    ...
    def get_instrument_property(self,obj,settings,debug=False):
        value=obj.query(settings['query_string'],return_bytes=None if 'return_bytes' not in settings else settings['return_bytes']+2)
    ...
    def set_values_property(self,obj,new_value,settings):
        ...
        if new_value not in values:
            ...
            return
        retval=self.query(settings['write_string'].format(new_value) if 'send_type' not in settings else settings['write_string'].format(settings['send_type'](new_value)),return_bytes=settings['ok_bytes']+2 if 'ok_bytes' in settings else None)
        return retval
    ...
    def initialize_properties(self):
        ...
        self.add_device_property({
            ...
            'query_string':'0707',
            'return_bytes':24,
            'return_type':lambda x: {a:b for a,b in zip(['RF','VCO','MCU','OSC','PA','USB'],unpack('ffffff',x))}}) # NOT TESTED

        def _get_state(x):
            """used in [state] property"""
            n0,n1=x#[int(w.hex(),16) for w in x]
            state=['Generator','Square modulation','Beacon modulation','Script modulation','Scanning','Control script'][n0]
            substate=''
            if 1<=n0<=3 and n1>0:
    ...
    def packet_checksum(self,rawpacket):
        ...
        return (sum(-int(s,16) for s in [hrawpqt[i:i+2] for i in range(0,len(hrawpqt),2)])%256+int('ff',16))%256
        # return bytes.fromhex('%02x'%((sum(-int(s,16) for s in [hrawpqt[i:i+2] for i in range(0,len(hrawpqt),2)])%256+int('ff',16))%256))
    ...
    def _bytes_to_int(self,bytcode,signed=False):
        ...
        if (not signed) and int(''.join([bytcode.hex()[i:i+2] for i in range(0,len(bytcode.hex()),2)][::-1]),16)!=int.from_bytes(bytcode,byteorder='little',signed=signed):
        ...
        return int(''.join([bytcode.hex()[i:i+2] for i in range(0,len(bytcode.hex()),2)][::-1]),16)
