...
class BaslerCamera(PylonSDK):
    ...
    Properties:
        ...
        gain
        sets/queries the camera gain in dB
        values  0.0 to 24.0
        return (float)

        width
        sets/queries the camera image width in pixels
        values  4, 1928
        return (int)

        height
        sets/queries the camera image height in pixels
        values  1, 1208
        return (int)

        pixel_format
        sets/queries the pixel depth/format
        values  ['Mono8', 'Mono12', 'Mono12p']
        return (str)

        exposure_time
        sets/queries the exposure_time
        values  ['Mono8', 'Mono12', 'Mono12p']
        return (str)
    ...
    def __init__(self, default_config=True, verbose=False, debug=False):
        ...
        self.instrument.RegisterConfiguration(
