...
def assign_pytorch_device(
    ...
    * Not possible to have all types of GPUs simultaneously available in the same environment; each requires a distinct
    Pytorch version
...
T_Optim = TypeVar("T_Optim", bound=torch.optim.Optimizer)
# LBFGS, Adam; LBFGS generally converges faster, but uses
# more memory; The reverse is true of Adam
DEFAULT_OPTIMIZERS = {
...
def make_optimizer(
    ...
    **kwargs
    ...
    # only pass kwargs that are in the signature
    return optim_class(params, **filtered_kwargs)


### custom mean function, although using the ConstantMean() is considered best practice if little to no information is
# known about the surface being probed


# *** only expects 2 independent variables and 1 dependent variable output; can make more generalized to handle
# arbitrary dimension inputs/outputs later
